      ...
      <div className="relative w-full max-w-[1400px]">
        ...
        >
            ...
            <div className="flex items-center space-x-2">
              <div className="flex items-center space-x-3 w-full">
                ...
                <div className="relative w-48 h-6 bg-gray-300 rounded-full overflow-hidden flex items-center">
                  <div className="bg-blue-600 h-full transition-all duration-300" style={{ width: `${expProgress}%` }}></div>
          ...
          <div className="flex flex-col space-y-1 p-1">
            <StatusBar icon="icon/waterIcon.png" label="어항 수질" value={aquariumData?.waterStatus ?? 0} color="bg-blue-900" />
            <StatusBar icon="icon/cleanIcon.png" label="청결도" value={aquariumData?.pollutionStatus ?? 0} color="bg-indigo-400" />
    ...
    <div className="flex items-center space-x-3">
      ...
      <div className="w-40 md:w-48 h-4 md:h-5 flex border-2 border-black rounded-full overflow-hidden">
        {Array.from({ length: segmentCount }).map((_, index) => (
          <div key={index} className={`flex-1 border-l border-black ${index < activeSegments ? color : "bg-white"} ${index === 0 ? "rounded-l-full" : ""} ${index === segmentCount - 1 ? "rounded-r-full" : ""}`} />
        ))}
      </div>
    </div>
  );

...
function FishOverlayModal({ aquariumId, onConfirm, onClose }: FishOverlayModalProps) {
  ...
  useEffect(() => {
    console.log("[FishOverlayModal] useEffect triggered for aquariumId:", aquariumId);
  ...
  const increment = (fish: string, max: number) => {
    ...
    setSelectedCounts((prev) => {
      ...
      if (current < max) {
        ...
        console.log(
          `[FishOverlayModal] ${fish} count increased from ${current} to ${current + 1}. New counts:`,
          newCounts
        );
        return newCounts;
      }
  ...
  const decrement = (fish: string) => {
    ...
    setSelectedCounts((prev) => {
      ...
      if (current > 0) {
        const newCounts = { ...prev, [fish]: current - 1 };
        console.log(
          `[FishOverlayModal] ${fish} count decreased from ${current} to ${current - 1}. New counts:`,
          newCounts
        );
        return newCounts;
      }
          ...
          <div className="max-h-60 overflow-y-auto mb-4">
            {groupedFish.length === 0 ? (
              ...
              groupedFish.map((group) => {
                ...
                return (
                  <div key={group.fish} className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      <img
                        ...
                        onError={(e) => {
                          console.error(`[FishOverlayModal] 이미지 로드 실패: ${group.fishImage}`, e);
                        }}
                        onLoad={() => {
                          console.log(`[FishOverlayModal] 이미지 로드 성공: ${group.fishImage}`);
                        }}
                      />
                      <span>
                    ...
                    <div className="flex items-center">
                      <button onClick={() => decrement(group.fish)} className="px-2 py-1 bg-gray-300 rounded-l">
                        -
                      </button>
        ...
        <div className="flex justify-end space-x-2">
          ...
          <button
            onClick={() => {
              ...
              console.log("[FishOverlayModal] 확인 버튼 클릭 - onConfirm 호출. 선택된 데이터:", selectedArray);

...
export default function FriendList({ userName, roomId, isHost, participantCount, users, onInvite }: FriendListProps) {
  ...
  const filteredFriends = friends.filter(
    (friend) => friend.nickname.includes(searchQuery) || friend.friendId.includes(searchQuery)
  );
    ...
    <div className="relative p-4 bg-transparent w-[300px] h-[500px] flex flex-col">
        ...
        {!isHost ? (
          ...
          <ul className="space-y-2">
            {filteredFriends.map((friend) => {
              const isJoined = users.some((user) => user.userName === friend.friendId);
      ...
      {isHost && (
        <div className="mt-4 flex items-center">
          ...
          />
          <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-r-md hover:bg-blue-700">검색</button>
        </div>
      )}
    </div>
  );

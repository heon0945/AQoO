  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        `/chatrooms?userId=${encodeURIComponent(userName)}`
      );
      ...
      for (const participant of participants) {
        ...
        try {
          ...
          console.error(
            `${participant.friendId}님 초대 중 에러 발생:`,
            inviteError
          );
        }
      }
    ...
    >
      ...
      >
        <div className='hidden md:flex gap-6 p-6 bg-white bg-opacity-30 border border-black rounded-lg shadow-lg w-[800px] h-[500px] relative justify-center items-center md:mt-20'>
          ...
          <div
            className='absolute top-[-40px] left-1/2 transform -translate-x-1/2 px-6 py-2 bg-white/70 border border-black rounded-lg shadow-lg'
            style={{
              backdropFilter: 'blur(5px)', // 뒤쪽 요소 흐리게 처리
            }}
          >
